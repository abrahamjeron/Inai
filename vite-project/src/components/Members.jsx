        ...
        <div className="bg-white p-2 md:p-0 rounded-lg md:rounded-none shadow md:shadow-none w-full md:w-auto md:min-w-[270px]">
            <div className="mt-3 md:mt-7">
                ...
                <div className="flex flex-col w-full md:w-[280px] items-center mt-2">
                    <h1 className="font-bold self-center text-[1rem] md:text-[1.2rem]">{roomName}</h1>
                    <p className="text-[#98A1B8] text-[0.8rem] md:text-[0.9rem]">{participants.length} Members</p>
                </div>
            </div>
            <div className="mt-3 md:mt-5 -ml-0 md:-ml-1">
                <div className="w-full md:w-[270px] max-w-md p-2 md:p-3">
                    <div className={`space-y-1 md:space-y-2 ${participants.length > 5 ? "max-h-[200px] md:max-h-[250px] rounded-2xl overflow-y-auto" : ""}`}>
                        {participants.map((participant, index) => (
                            <div 
                                key={`${participant.id}-${index}`} 
                                className="flex items-center justify-between rounded-xl p-1 group hover:bg-gray-800 transition"
                            >
                                <div className="flex items-center gap-1 md:gap-2">
                                    ...
                                    <div>
                                        <h3 className="text-black group-hover:text-white font-semibold transition text-xs md:text-sm">
                                            {participant.displayName}
                                        </h3>
                                        <p className="text-gray-400 text-xs group-hover:text-gray-300 transition">
                                            {participant.role}
                                ...
                                <button
                                    ...
                                    className={`px-1 md:px-2 py-0.5 rounded-full text-xs font-medium transition
                                        ${participant.isHost 
                                            ? 'bg-black text-white border border-white/20 group-hover:bg-white group-hover:text-black' 
                                            : 'bg-black text-white border border-white/20 group-hover:bg-white group-hover:text-black'
                                        }`}
            ...
            <div className="flex justify-center md:justify-start md:ml-3 mt-2">
                <button
                    onClick={leaveRoom}
                    className="bg-red-500 text-black w-[80px] md:w-[50px] p-1 mr-2 rounded text-xs"
                >
                    Leave Room
                </button>
            </div>
